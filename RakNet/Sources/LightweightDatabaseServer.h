...
class RAK_DLL_EXPORT LightweightDatabaseServer : public PluginInterface2
	{
	public:
		typedef std::map<std::string,unsigned short> IPMap;
		typedef std::list<std::string> IPList;
		struct InternalID { int port; IPList ips; };
		typedef std::map<SystemAddress, InternalID> AddressMap;
		// Constructor
		...
		void SetStatDelay(int value) { statDelay = value; }
		int GetStatDelay() { return statDelay; }
		...
		LightweightDatabaseServer::DatabaseTable* CreateDefaultTable(std::string tableName);
	...
	protected:
		...
		DatabaseTable * DeserializeClientHeader(RakNet::BitStream *inBitstream, Packet *packet, int mode, bool query = false);
		...
		bool CheckVersion(Packet *packet, RakNet::BitStream &inBitStream);
		IPMap ipMap;
		AddressMap addressMap;
		char version[3];
		int lastMinQueryCount;
		int lastMinUpdateCount;
		time_t countTimer;
		time_t statTimer;
		int statDelay;	// Delay between displaying statistics
	};
