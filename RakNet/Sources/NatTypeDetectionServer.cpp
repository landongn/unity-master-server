...
bool NatTypeDetectionServer::OnDetectionRequest(Packet *packet)
{
	#if defined(_DEBUG)
	printf("Got detection request, last ping was %d\n", rakPeerInterface->GetLastPing(packet->systemAddress));
	#endif
	if (rakPeerInterface->GetLastPing(packet->systemAddress) == 65535)
	{
