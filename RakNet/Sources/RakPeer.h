...
//		int connectionSocketIndex; // index into connectionSockets to send back on.
		...
		RakNetGUID guid;
		int MTUSize;
		// The local port which received communication from this remote system
	...
	char* HandleRPCPacket( const char *data, int length, SystemAddress systemAddress );	
...
protected:
	...
	*/
	...
	RemoteSystemStruct * AssignSystemAddressToRemoteSystemList( const SystemAddress systemAddress, RemoteSystemStruct::ConnectMode connectionMode, RakNetSmartPtr<RakNetSocket> incomingRakNetSocket, bool *thisIPConnectedRecently, SystemAddress bindingAddress, int incomingMTU, RakNetGUID guid, unsigned short rcvPort );
